...
Try {
	...
	[string]$appVendor = 'Robin Krom'
	[string]$appName = 'Greenshot'
	[string]$appVersion = '1.2.10.6'
	[string]$appArch = 'x86'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.7.3'
	[string]$appScriptDate = '2022.08.03'
	...
	$CloseApps = "Greenshot,msedge=Microsoft Edge,firefox=Firefox,chrome=Google Chrome"
	...
	$64bitAppInstallName = "Greenshot"
	# Application install Path.
	$64bitAppInstallPath = "$PSScriptRoot\Files\Greenshot-INSTALLER-1.2.10.6-RELEASE.exe"
	# Application  install parameters.
	$64bitAppInstallParam = "/NORUN /SUPPRESSMSGBOXES /VERYSILENT /NOCANCEL /NORESTART" 
	...
	#$RemoveAppNamesMSI = @("")
	$RemoveAppNamesEXE = @("Greenshot")

	##* Application uninstall parameters.
	# 64-bit application
	# 64-bit application uninstall name.
	$64bitAppUninstallName = "Greenshot"
	# Application uninstall path.
	$64bitAppUninstallPath = "$Env:ProgramFiles\Greenshot\unins000.exe"
	# Application uninstall parameters.
	$64bitAppUninstallParam = "/NORUN /SUPPRESSMSGBOXES /VERYSILENT /NOCANCEL /NORESTART"
	...
	$appSettingsDir = "$Env:ProgramData\Tuolumne County\Application Settings\Greenshot"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		If (Test-Path -Path "HKLM:SOFTWARE\Microsoft\Active Setup\Installed Components\Greenshot") {
			# Remove previous versions of activesetup key.
			Try {
				Set-ActiveSetup -Key "Greenshot" -PurgeActiveSetupKey
		...
		Try {
			Write-Log -Message "Creating settings directory." 
			New-Item -Path "$appSettingsDir" -Type "directory"
		}
		# If copy failed, log results. Exit script.
		Catch [System.Exception] {
		...
		# Copy settings file and scipt to settings directory.
        # If copy failed, log results. Exit script.
        Try {
            Write-Log -Message "Copying settings file and scipt to settings directory." 
            Copy-Item -Path "$PSScriptRoot\Support Files\Greenshot.ini" -Destination "$appSettingsDir" -Force
            Copy-Item -Path "$PSScriptRoot\Support Files\Copy Greenshot Settings.cmd" -Destination "$appSettingsDir" -Force
        }
        # Copy failed, log results. Exit script. 
        Catch [System.Exception] {
            Write-Log -Message "Copy of settings file and scipt to settings directory failed with error: $_."
            Write-Log -Message "Exiting script with error."
            Exit-Script -ExitCode 1627
        }
		...
		Try {
			...
			Set-ActiveSetup -StubExePath "$envWinDir\system32\cmd.exe" -Arguments "/C `"$Env:ProgramData\Tuolumne County\Application Settings\Greenshot\Copy Greenshot Settings.cmd`"" -Description "Greenshot Default Settings" -Key "Greenshot"
			Set-RegistryKey -Key "HKLM:\SOFTWARE\Microsoft\Active Setup\Installed Components\Greenshot" -Name "Readme" -Value "This key launches Copy Greenshot Settings.cmd, which copies the greenshot default settings file to the greenshot settings folder for each user." -Type String
